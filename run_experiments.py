...
DATASET_BANDWIDTH_DIFF_NI_POW = 'experiments/bandwidth_cost_diff_ni_pow.csv'
...
async def calc_total_size(client: ZcashClient, row: pd.Series, opt_lv: FlyclientBenchmark._OPT_TYPE, enc_type: FlyclientBenchmark._ENC_TYPE = 'normal'):
    ...
    size = benchmark.calculate_total_download_size_bytes(optimization=opt_lv, encoding=enc_type)
    print(f"Calculated size for {row.chaintip, row.c, row.L}, opt_lv={opt_lv} => {size / 2**20:.3} MiB")
    return size

async def gen_dataset(file_path: str, samples_file: str, type: __DATASET_TYPE, enc_type: FlyclientBenchmark._ENC_TYPE = 'normal'):
    ...
    async def bounded_calc(client: ZcashClient, row: pd.Series, opt_lv: FlyclientBenchmark._OPT_TYPE):
        async with semaphore:
            return await calc_total_size(client, row, opt_lv, enc_type)
...
async def main():
    ...
    if not os.path.isfile(DATASET_BANDWIDTH_DIFF_NI_RC):
        await gen_dataset(DATASET_BANDWIDTH_DIFF_NI_RC, SAMPLES_BANDWIDTH_DIFFICULTY_NI, 'bandwidth', 'rice_coding')
    
    if not os.path.isfile(DATASET_BANDWIDTH_DIFF_NI_POW):
        await gen_dataset(DATASET_BANDWIDTH_DIFF_NI_POW, SAMPLES_BANDWIDTH_DIFFICULTY_NI, 'bandwidth', 'flyclient_friendly')
